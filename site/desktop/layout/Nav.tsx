...
export default function Nav({ config, lang }: { config: Record<string, any>; lang: string }) {
    ...
    const getContent = () => {
        return nav.map((group: { title: React.ReactNode; items: any[]; }, index: string | number | undefined) => {
            return (
                // eslint-disable-next-line react/jsx-key
                <div key={index} className="mul-doc-nav__group">
                    <div className="mul-doc-nav__title">{group.title}</div>
                    {group.items.length > 0 &&
                        group.items.map((item: { title: any; name: any; path: string; }, itemIndex: string | number | undefined) => {
